...
export function ok(data, init={}){
  return new Response(JSON.stringify({ok:true, data}), {status:200, headers:{'Content-Type':'application/json', ...init.headers}});
}
...
export async function rateLimit(req, keyBase, max=60){
  ...
  const key = `rl/${keyBase}-${ip}.json`;
  let rec = await s.getJSON(key) || {ts:[], updated_at:0};
  rec.ts = (rec.ts||[]).filter(t=> now - t < windowMs);
  if(rec.ts.length >= max){ return {allowed:false, remaining:0}; }
  rec.ts.push(now);
  rec.updated_at = now;
  await s.setJSON(key, rec);
  return {allowed:true, remaining:Math.max(0, max - rec.ts.length)};
}
...
export function isAdmin(user){
  return !!user && (user.phone === ADMIN_PHONE && ADMIN_PHONE);
}
