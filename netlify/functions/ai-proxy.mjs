...
const MODEL = process.env.UPSTREAM_MODEL || 'gpt-4o-mini';
const MAX_REQ_10MIN = parseInt(process.env.MAX_REQ_10MIN||'60',10);

export default async (req) => {
  let user=null;
  const needAuth = process.env.REQUIRE_AUTH !== '0';
  if(needAuth){
  ...
  const keyBase = user? ('ai-'+user.uid) : 'ai-public';
  const rl = await rateLimit(req, keyBase, MAX_REQ_10MIN);
  if(!rl.allowed){
    await auditLog({route:'ai-proxy', uid: user?.uid||'', phone:user?.phone||'', ok:false, why:'rate_limit'});
  ...
  try{
    ...
    if(!okRes) return new Response(txt, {status:502, headers:{'Content-Type':'application/json'}});
    return new Response(JSON.stringify(data), {status:200, headers:{'Content-Type':'application/json'}});
  }catch(e){
    await auditLog({route:'ai-proxy', uid:user?.uid||'', phone:user?.phone||'', ok:false, why:'fetch_error', err:String(e)});
    return fail('代理失败', 502);
  }
}
